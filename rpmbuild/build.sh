...
function check() {
    ...
    if [ $VALIDATE -gt 0 ]; then
        get_version icinga_check_bigip_pool
        if [ "${COMBINED}" == "${VERSION}-${RELEASE}" ]; then
            log 1 "There is already a icinga_check_bigip_pool RPM with version ${VERSION} and release ${RELEASE}. Bump version/release before rebuilding."
            exit 3
        fi
    fi
}
